#!/usr/bin/env bash
# ============================================================================
# Vault E2E integration test runner
# ============================================================================
...
# are deleted after the test. The test itself refuses any target that is not
# an HTTP loopback disposable server (see _assertDisposableTarget).
#
# Usage:
#   chmod +x test/vault_integration_run.sh
...
echo "=== Vault E2E Test Runner ==="

# -- Create temp paths --------------------------------------------------------
export BMO_TEST_DB
BMO_TEST_DB=$(mktemp /tmp/bmo-e2e-vault-XXXXXX.db)
echo "[1/6] Temp DB: $BMO_TEST_DB"

export BMO_TEST_BLOBS
BMO_TEST_BLOBS=$(mktemp -d /tmp/bmo-e2e-blobs-vault-XXXXXX)
